...
#include "src\airplane\AirplaneSceneObject.h"
#include "src\magicStone\MagicStoneSceneObject.h"
...
AirplaneSceneObject* m_airplane = nullptr;
MagicStoneSceneObject* m_magicStone = nullptr;
...
bool initializeGL(){
	...
	// 1. defaultRenderer(GeomeryPass)
	// 2. deferredRenderer(DeferredPass)
	// ================================================
	...
	// 2. deferredRenderer(DeferredPass)
	// vertex shader
	Shader* vsDeferredShader = new Shader(GL_VERTEX_SHADER);
	vsDeferredShader->createShaderFromFile("src\\shader\\oglDeferredVertexShader.glsl");
	std::cout << vsDeferredShader->shaderInfoLog() << "\n";

	// fragment shader
	Shader* fsDeferredShader = new Shader(GL_FRAGMENT_SHADER);
	fsDeferredShader->createShaderFromFile("src\\shader\\oglDeferredFragmentShader.glsl");
	std::cout << fsDeferredShader->shaderInfoLog() << "\n";

	// shader program
	ShaderProgram* deferredShaderProgram = new ShaderProgram();
	deferredShaderProgram->init();
	deferredShaderProgram->attachShader(vsDeferredShader);
	deferredShaderProgram->attachShader(fsDeferredShader);
	deferredShaderProgram->checkStatus();
	if (deferredShaderProgram->status() != ShaderProgramStatus::READY) {
		return false;
	}
	deferredShaderProgram->linkProgram();

	vsDeferredShader->releaseShader();
	fsDeferredShader->releaseShader();

	delete vsDeferredShader;
	delete fsDeferredShader;
	...
	if (!deferredRenderer->initialize(FRAME_WIDTH, FRAME_HEIGHT, deferredShaderProgram)) {
		return false;
	}
	// =================================================================
	...
	// initialize airplane
	m_airplane = new AirplaneSceneObject();
	defaultRenderer->appendAirplaneSceneObject(m_airplane);

	// initialize magic stone
	m_magicStone = new MagicStoneSceneObject();
	defaultRenderer->appendMagicStoneSceneObject(m_magicStone);
	// =================================================================	
...
void paintGL(){
	...
	// form ImGui
		// update teleport
	if (m_imguiPanel->getTeleport() != notSpecified) {
		m_myCameraManager->teleport(m_imguiPanel->getTeleport());
		m_imguiPanel->resetTeleport();
	}
		// update deferred output type
	deferredRenderer->setDeferredOutput(m_imguiPanel->getGBUFFER());
		// update isNotmalMapping
	defaultRenderer->setIsNormalMapping(m_imguiPanel->getIsNormalMapping());
	...
	// update position of airplane
	m_airplane->updateState(airplaneModelMat);
