...
namespace INANOA {
	namespace OPENGL {
		...
		void RendererBase::useProgram() {
			this->m_shaderProgram->useProgram();
		}

		GLuint RendererBase::getUniformLocation(const std::string s) {
			return glGetUniformLocation(this->m_shaderProgram->programId(), s.c_str());
		}
